...
def test_get_by_id_returns_expense():
    ...
    """
    repo = InMemoryExpenseRepository()
    expense = create_expense(id=10, title="Cena", amount=30)
    repo.save(expense)
    retrieved_expense = repo.get_by_id(10)
    assert retrieved_expense is not None
    assert retrieved_expense.id == 10
    assert retrieved_expense.title == "Cena"
    assert retrieved_expense.amount == 30
...
def test_get_by_id_returns_none_if_not_found():
    ...
    """
    repo = InMemoryExpenseRepository()
    retrieved_expense = repo.get_by_id(999)
